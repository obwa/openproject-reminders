...
class reminder < ActiveRecord::Base
  ...
  # Returns true if usr or current user is allowed to view the reminder
  def visible?(user = nil)
    (user || User.current).allowed_to?(:view_reminders, project)
  end
  ...
  def all_changeable_participants
    ...
    changeable_participants = changeable_participants + \
                              User.allowed_members(:view_reminders, project)
  ...
  def self.group_by_time(reminders)
    ...
    reminders.group_by(&:start_year).each do |year, objs|
