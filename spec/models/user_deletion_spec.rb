...
describe User, '#destroy', type: :model do
  ...
  let(:reminder) {
    FactoryGirl.create(:reminder, project: project,
                                 author: user2)
  }
  let(:participant) {
    FactoryGirl.create(:reminder_participant, user: user,
                                             reminder: reminder,
                                             invited: true,
                                             attended: true)
  }
  ...
  describe 'WHEN the user created a reminder' do
    let(:associations) { [:author] }
    let(:associated_instance) { FactoryGirl.build(:reminder, project: project) }
    let(:associated_class) { reminder }

    it_should_behave_like 'created journalized associated object'
  end

  describe 'WHEN the user updated a reminder' do
    let(:associations) { [:author] }
    let(:associated_instance) { FactoryGirl.build(:reminder, project: project) }
    let(:associated_class) { reminder }
  ...
  describe 'WHEN the user created a reminder agenda' do
    let(:associations) { [:author] }
    let(:associated_instance) {
      FactoryGirl.build(:reminder_agenda, reminder: reminder,
                                         text: 'lorem')
    }
    let(:associated_class) { reminderAgenda }

    it_should_behave_like 'created journalized associated object'
  end

  describe 'WHEN the user updated a reminder agenda' do
    let(:associations) { [:author] }
    let(:associated_instance) {
      FactoryGirl.build(:reminder_agenda, reminder: reminder,
                                         text: 'lorem')
  ...
  describe 'WHEN the user created a reminder minutes' do
    let(:associations) { [:author] }
    let(:associated_instance) {
      FactoryGirl.build(:reminder_minutes, reminder: reminder,
                                          text: 'lorem')
    }
    let(:associated_class) { reminderMinutes }

    it_should_behave_like 'created journalized associated object'
  end

  describe 'WHEN the user updated a reminder minutes' do
    let(:associations) { [:author] }
    let(:associated_instance) {
      FactoryGirl.build(:reminder_minutes, reminder: reminder,
                                          text: 'lorem')
  ...
  describe 'WHEN the user participated in a reminder' do
    before do
      participant
    ...
    it { expect(reminder.participants.map(&:user)).to match_array([DeletedUser.first, user2]) }
