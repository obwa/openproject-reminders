...
describe remindersController, type: :controller do
  ...
  describe 'GET' do
    describe 'index' do
      before(:each) do
        @ms = [mock_model(reminder), mock_model(reminder), mock_model(reminder)]
        allow(@ms).to receive(:from_tomorrow).and_return(@ms)

        allow(project).to receive(:reminders).and_return(@ms)
        [:with_users_by_date, :page, :per_page].each do |meth|
          expect(@ms).to receive(meth).and_return(@ms)
        end
        @grouped = double('grouped')
        expect(reminder).to receive(:group_by_time).with(@ms).and_return(@grouped)
      end
      describe 'html' do
        before(:each) do
        ...
        it { expect(assigns(:reminders_by_start_year_month_date)).to eql @grouped }
      end
    end

    describe 'show' do
      before(:each) do
        @m = mock_model(reminder)
        allow(reminder).to receive_message_chain(:includes, :find).and_return(@m)
    ...
    describe 'new' do
      before(:each) do
        ...
        allow(reminder).to receive(:new).and_return(@m)
      end
      describe 'html' do
        before(:each) do
        ...
        it { expect(response).to be_success }
        it { expect(assigns(:reminder)).to eql @m }
      end
    end
